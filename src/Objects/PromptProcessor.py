...
class PromptProcessor:
    ...
    def generate_fn_name(self, prompt: Prompt) -> Any:
        ...
        while True:
            prompt_message = (
                ...
                f'{available_functions}. To resolve, "{prompt.prompt}".'
            )
    ...
    def generate_parameters(
        self, prompt_message: Prompt, function_name: str
    ) -> Dict[Any, Any]:
        ...
        for param in definition.parameters:
            ...
            for arg in output.keys():
                previous_gen = (
                    previous_gen + arg + "=" + str(output[arg]) + "\n"
                )
    ...
    def generate_int_parameter(
        self, prompt_message: Prompt, function:
        FunctionDefinition, previous_gen: str
    ) -> float:
        """Generate a float parameter."""
        prompt = (
            f"To solve the prompt {prompt_message}, you will use: "
            f"{function.full_definition}. Provide each parameter. Don't add "
            "custom fields."
        )
        ...
        while True:
            for generation in self.__llm.predict_multiple_tokens(
                ...
                if (argument_progress + generation).count("-") == 1 and (
    ...
    def generate_str_parameter(
        ...
        prompt = (
            f"To solve the prompt {prompt_message}, you will use: "
            f"{function.full_definition}. Provide each parameter. "
        ...
        while "\n" not in argument_progress:
            generation = self.__llm.predict_token(
                prompt_message=prompt, previous_tokens=previous_gen +
                argument_progress
            )
            if generation == "":
